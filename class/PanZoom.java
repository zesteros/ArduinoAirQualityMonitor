...
public class PanZoom implements View.OnTouchListener {
    ...
    private Number minX, maxX, minY, maxY;
    ...
    protected PanZoom(XYPlot plot, Pan pan, Zoom zoom, Number minX, Number maxX, Number minY, Number maxY ) {
        this.minX = minX;
        this.maxX = maxX;
        this.minY = minY;
        this.maxY = maxY;
     ...
     * Use  for finer grain control of this behavior.
     * @param plot
     * @return
     */
    public static PanZoom attach(XYPlot plot, Number minX, Number maxX, Number minY, Number maxY) {
        return attach(plot, Pan.BOTH, Zoom.SCALE, minX, maxX, minY, maxY);
    }

    public static PanZoom attach(XYPlot plot, Pan pan, Zoom zoom, Number minX, Number maxX, Number minY, Number maxY) {
        PanZoom pz = new PanZoom(plot, pan, zoom,  minX, maxX, minY, maxY);
    ...
    protected void setDomainBoundaries(final Number lowerBoundary, final BoundaryMode lowerBoundaryMode,
        ...
        if(mCalledBySelf) {
            ...
            final RectRegion bounds = new RectRegion(minX, maxX, minY, maxY);
...
final Number upperBoundary, final BoundaryMode mode) {
    ...
    private float getMinXLimit() {
        if(minXLimit == Float.MAX_VALUE) {
            minXLimit = new RectRegion(minX, maxX, minY, maxY).getMinX().floatValue();
            lastMinX = minXLimit;
        }
        return minXLimit;
    }
    ...
    protected float getMaxXLimit() {
        if(maxXLimit == Float.MAX_VALUE) {
            maxXLimit = new RectRegion(minX, maxX, minY, maxY).getMaxX().floatValue();
            lastMaxX = maxXLimit;
        }
        return maxXLimit;
    }
    ...
    protected float getMinYLimit() {
        if(minYLimit == Float.MAX_VALUE) {
            minYLimit = new RectRegion(minX, maxX, minY, maxY).getMinY().floatValue();
            lastMinY = minYLimit;
        }
        return minYLimit;
    }
    ...
    protected float getMaxYLimit() {
        if(maxYLimit == Float.MAX_VALUE) {
            maxYLimit = new RectRegion(minX, maxX, minY, maxY).getMaxY().floatValue();
            lastMaxY = maxYLimit;
        }
        return maxYLimit;
    }
    ...
    protected float getLastMinX() {
        if(lastMinX == Float.MAX_VALUE) {
            lastMinX = new RectRegion(minX, maxX, minY, maxY).getMinX().floatValue();
        }
        return lastMinX;
    }
    ...
    protected float getLastMaxX() {
        if(lastMaxX == Float.MAX_VALUE) {
            lastMaxX = new RectRegion(minX, maxX, minY, maxY).getMaxX().floatValue();
        }
        return lastMaxX;
    }
    ...
    protected float getLastMinY() {
        if(lastMinY == Float.MAX_VALUE) {
            lastMinY = new RectRegion(minX, maxX, minY, maxY).getMinY().floatValue();
        }
        return lastMinY;
    }
    ...
    private float getLastMaxY() {
        if(lastMaxY == Float.MAX_VALUE) {
            lastMaxY = new RectRegion(minX, maxX, minY, maxY).getMaxY().floatValue();
        }
        return lastMaxY;
    }
